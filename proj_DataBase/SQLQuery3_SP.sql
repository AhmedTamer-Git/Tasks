...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM exampool e
        JOIN question q ON e.que_id = q.que_id
...
--------------------------------
CREATE PROCEDURE sp_GetStudentExamResult
    @StudentID INT,
    @ExamID INT
with encryption 
AS
BEGIN
    ...
    SELECT 
        ...
        END AS resultStatus
    FROM Exam e
    JOIN stu_takes_exam ste ON e.Exam_id = ste.Exam_id
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (
            SELECT 1 FROM Course_Teach_By_Instructor
        ...
        BEGIN
            INSERT INTO exampool (exam_id, que_id, CourseID, timedate)
            SELECT 
                @ExamID,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM Training_manager WHERE manId = @ManagerID)
    BEGIN
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM Student WHERE IntakeID = @IntakeID)
        BEGIN
        ...
        DELETE FROM TrackIntake WHERE IntakeID = @IntakeID;
...
CREATE TRIGGER trg_check_unanswered_questions
ON stu_takes_exam
AFTER UPDATE
AS
BEGIN
...
CREATE TRIGGER trg_answer_validation
ON answer
INSTEAD OF INSERT
AS
BEGIN
